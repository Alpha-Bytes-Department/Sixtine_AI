...
const UserDetails = () => {
  ...
  const handleGoBack = () => {
    ...
    const { setPageTitle, setBreadCrumb, isEnglish ,breadCrumb} = useStatus();
   useEffect(() => {
     setPageTitle(isEnglish ? "User Details" : "Détails de l'utilisateur");
     setBreadCrumb([
       { title: isEnglish ? "User" : "Utilisateur", link: "/dashboard/users" },
       { title: isEnglish ? "User Details" : "Détails de l'utilisateur", link: "/dashboard/users/0" },
     ]);
   }, [ isEnglish]);
  ...
  return (
    <>
      ...
      <div className="flex-1  flex justify-center items-center">
        <div className="shadow-[0_0_4px_0_#00000040] px-3  lg:px-36 py-5 mx-auto lg:py-12 relative">
          <button onClick={handleGoBack}>
            <RxCross2 className="absolute top-5 right-5 text-xl text-[#4e7ba0] cursor-pointer" />
          </button>
          <div className="flex justify-center flex-col gap-5 text-[#4A4A4A] ">
